...
__global__ void generate_splitters(double *E, int k, int spl_size, int *lower_splitter, int *upper_splitter, curandState *state) {
	...
	while (1) {
		...
		if (threadIdx.x < k) {
			first_greater_than_pivot[threadIdx.x] = binary_search_greater_than(E, lower_search_boundary[threadIdx.x],    upper_search_boundary[threadIdx.x]-1, pivot);
			first_smaller_than_pivot[threadIdx.x] = binary_search_smaller_than(E, lower_search_boundary[threadIdx.x], first_greater_than_pivot[threadIdx.x]-1, pivot);
	...
	if (threadIdx.x < k) {
		pivot_count_per_list[threadIdx.x + bank_offset] = first_greater_than_pivot[threadIdx.x] - first_smaller_than_pivot[threadIdx.x] - 1;
	}
...
__global__ void gpu_merge(double *E, int k, int *splitters, double *W0, double *W1, int n) {
	...
	int sublist_start_position = blockIdx.x *(n/SUBLIST_COUNT) + (blockIdx.x <= n % SUBLIST_COUNT ? blockIdx.x : n % SUBLIST_COUNT);
